...
import space_back.service.MoteurPhysique;
...
class MoteurPhysiqueTest {
    ...
    // -----------------------------------------------------------------------
    // Helpers : construction des mocks IDAOCelestialBody
    // -----------------------------------------------------------------------

    /** CelestialBody placé à l'origine avec la masse du Soleil. */
    private static CelestialBody soleil() {
        return new CelestialBody("Soleil", new double[]{0.0, 0.0}, M_SUN);
    }
    ...
    private static CelestialBody bodyOnX(String name, double x, double mass) {
        return new CelestialBody(name, new double[]{x, 0.0}, mass);
    }
     ...
     * via un IDAOCelestialBody mocké.
     */
    ...
    class ComputeAccelerationTests {
        ...
        /**
         * 1.1 — Attraction du Soleil seul.
         * Vaisseau à 1 UA sur l'axe X. Le vecteur retourné doit pointer vers
         * l'origine et sa norme doit valoir exactement G·M☉/r².
         *
         * DAO mocké : findAll() → [Soleil à (0,0)]
         */
        @Test
        ...
        /**
         * 1.2 — Superposition symétrique de deux corps.
         * Deux corps de même masse de part et d'autre du vaisseau sur l'axe X.
         * L'accélération résultante doit être nulle par symétrie.
         *
         * DAO mocké : findAll() → [CorpsGauche, CorpsDroite]
         */
        @Test
        ...
        void accelerationDeuxCorpsSymetriques_resultanteNulle() throws Exception {
            ...
            // Norme de chaque contribution individuelle (pour calibrer la tolérance)
            double singleContrib = G * mass / (d * d);
            double toleranceNull = singleContrib * 1e-9;

            assertEquals(0.0, acc[0], toleranceNull,
                    "ax doit être nul par symétrie");
            assertEquals(0.0, acc[1], toleranceNull,
                    "ay doit être nul par symétrie");
        }

        /**
         * 1.3 — Cas dégénéré : vaisseau exactement sur les coordonnées du Soleil.
         * Le guard (dist < 1.0) doit absorber la singularité et retourner [0,0]
        ...
        void accelerationSinguliere_retourneZero() {
            ...
            assertDoesNotThrow(() -> {
                double[] acc = computeAcceleration(moteur, 0.0, 0.0);
                assertEquals(0.0, acc[0], 1e-30,
                        "ax doit être 0 en cas dégénéré (dist < 1)");
                assertEquals(0.0, acc[1], 1e-30,
                        "ay doit être 0 en cas dégénéré (dist < 1)");
    ...
    class OrbitIntegrationTests {
        ...
        /**
         * 2.1 — Conservation du rayon orbital.
         * Après une révolution complète (2π), tous les points de la trajectoire
         * doivent rester à ±0.1 % de 1 UA.
         *
         * DAO mocké : findAll() → [Soleil à (0,0)]
         */
        @Test
        ...
        void conservationRayonOrbital() {
            ...
            double tolerance = ONE_AU * 0.001; // 0.1 %
         ...
         * E = ½(vx²+vy²) − G·M/r est une constante du mouvement.
         * La variance relative des énergies sur toute la trajectoire doit être < 1e-6.
         *
         * DAO mocké : findAll() → [Soleil à (0,0)]
         */
        @Test
        ...
        /**
         * 2.3 — Période orbitale vs 3ème loi de Kepler.
         * T_simulé = nb_points × dt doit être dans ±1 % de T_kepler.
         *
         * DAO mocké : findAll() → [Soleil à (0,0)]
         */
        @Test
        ...
        /**
         * 2.4 — Bouclage géométrique.
         * Le premier et le dernier point de la trajectoire doivent être
         * distants de moins de 500 km — le vaisseau revient à son point de départ.
         *
         * DAO mocké : findAll() → [Soleil à (0,0)]
         */
        @Test
    ...
    class PerturbateOrbitTests {
        ...
        /**
         * 3.1 — Préservation de l'historique.
         * Les points de trajectoire antérieurs à t_start doivent être
         * bit-à-bit identiques après l'appel à perturbateOrbit.
         *
         * DAO mocké : findAll() → [Soleil à (0,0)]
         */
        @Test
        ...
        void preservationHistorique() {
            ...
            // Snapshot de la trajectoire complète avant perturbation
            LinkedHashMap<Integer, double[]> trajAvant = (LinkedHashMap<Integer, double[]>) orbit.getTrajectoire().clone();
        ...
        /**
         * 3.2 — Remplacement du segment futur.
         * Après une perturbation de +1000 m/s sur vy, les points au-delà
         * de fromStep doivent différer de l'orbite originale.
         *
         * DAO mocké : findAll() → [Soleil à (0,0)]
         */
        @Test
        ...
        void remplacementSegmentFutur() {
            ...
            Orbit orbit = moteur.eulerOrbitInit(ONE_AU, 0.0, 0.0, V_CIRC_1AU);
            LinkedHashMap<Integer, double[]> trajOriginal = (LinkedHashMap<Integer, double[]>) orbit.getTrajectoire().clone();

            int fromStep = trajOriginal.size() / 2;
            double tStart = fromStep * moteur.getDt();

            // Perturbation : impulsion de +1000 m/s sur vy
        ...
        /**
         * 3.3 — Idempotence sans perturbation.
         * Rappeler perturbateOrbit avec le vecteur d'état exact et thetaWindow = 2π
         * doit reproduire une orbite très proche de l'originale (< 100 km d'écart
         * sur tous les points recalculés).
         *
         * DAO mocké : findAll() → [Soleil à (0,0)]
         */
        @Test
    ...
    class RobustnessTests {
        ...
        /**
         * 4.1 — thetaWindow = 0.
         * perturbateOrbit ne doit pas modifier l'orbite et ne doit pas
         * lever d'exception.
         *
         * DAO mocké : findAll() → [Soleil à (0,0)]
         */
        ...
        /**
         * 4.2 — Ratio thetaWindow π vs 2π.
         * Une demi-orbite doit contenir environ la moitié des points d'une orbite
         * complète (tolérance ±5 % pour la discrétisation).
         *
         * DAO mocké : findAll() → [Soleil à (0,0)]
         */
        @Test
        ...
        void ratioThetaWindowPiVs2Pi() {
            ...
            // Demi-orbite via perturbateOrbit avec thetaWindow = π
        ...
        /**
         * 4.3 — Appels successifs : absence d'accumulation de StepHandlers.
         * Trois appels eulerOrbitInit successifs sur la même instance doivent
         * produire des trajectoires bit-à-bit identiques.
         *
         * DAO mocké : findAll() → [Soleil à (0,0)]
         */
        @Test
        ...
        void appelsSucessifs_idempotents() {
            ...
            assertEquals(t1.size(), t2.size(),
                    "Orbites 1 et 2 doivent avoir la même taille");
            assertEquals(t2.size(), t3.size(),
                    "Orbites 2 et 3 doivent avoir la même taille");
        ...
        /**
         * 4.4 — Perturbations planétaires : Soleil seul vs six corps.
         * L'orbite avec six corps doit dériver de l'orbite keplerienne pure,
         * mais rester dans un ordre de grandeur physiquement sensé (< 10 000 km).
         *
         * Deux DAOs mockés distincts :
         *   - daoKep      : findAll() → [Soleil]
         *   - daoSixCorps : findAll() → [Soleil, Mercure, Venus, Mars, Jupiter, Saturne]
         */
        @Test
        @DisplayName("4.4 — Perturbations planétaires : dérive non nulle mais < 10 000 km")
        void perturbationsPlanetaires_deriveRaisonnable() {

            // --- Orbite keplerienne (Soleil seul) ---
            MoteurPhysique moteurKep = moteurSoleilSeul();
            Orbit orbitKep = moteurKep.eulerOrbitInit(ONE_AU, 0.0, 0.0, V_CIRC_1AU);

            // --- Orbite perturbée (Soleil + 5 planètes) ---
            List<CelestialBody> sixCorps = new ArrayList<>();
            sixCorps.add(soleil());
            sixCorps.add(new CelestialBody("Mercure", new double[]{5.79e10, 0.0}, 3.30e23));
            sixCorps.add(new CelestialBody("Venus",   new double[]{1.08e11, 0.0}, 4.87e24));
            sixCorps.add(new CelestialBody("Mars",    new double[]{2.28e11, 0.0}, 6.39e23));
            sixCorps.add(new CelestialBody("Jupiter", new double[]{7.78e11, 0.0}, 1.90e27));
            sixCorps.add(new CelestialBody("Saturne", new double[]{1.43e12, 0.0}, 5.68e26));
            ...
            // Physiquement sensé : la dérive est non nulle mais < 10 000 km
